...
const HeroSection = () => {
  const stats = [
    { label: 'Revenue', value: `฿${(dashboardStats.revenue / 1000).toFixed(0)}k`, sub: `+${dashboardStats.revenueChange}%`, icon: TrendingUp, positive: true },
    { label: 'Occupancy', value: `${dashboardStats.occupancy}%`, sub: `+${dashboardStats.occupancyChange}%`, icon: Home, positive: true },
    { label: 'Overbooking', value: '0', sub: 'Zero!', icon: ShieldCheck, positive: true },
    { label: 'AI Response', value: `${dashboardStats.aiResponseRate}%`, sub: 'Auto', icon: Zap, positive: true },
    { label: 'Tasks', value: `${dashboardStats.activeTasks}`, sub: 'Active', icon: ClipboardList, positive: false },
    { label: 'Inquiries', value: `${dashboardStats.pendingInquiries}`, sub: 'Pending', icon: MessageCircle, positive: false },
  ];
        ...
        <div className="absolute inset-0 hero-overlay flex flex-col justify-end p-6">
          ...
          <div className="flex items-center gap-3 mt-3">
            ...
            <span className="font-display text-xs text-primary-foreground/60">
              3 properties · {new Date().toLocaleDateString('th-TH', { day: 'numeric', month: 'short', year: 'numeric' })}
            </span>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-3 lg:grid-cols-6 gap-2">
        {stats.map((stat, idx) => (
          ...
          >
            ...
            <div className={`font-display text-[10px] font-semibold mt-0.5 ${stat.positive ? 'text-accent' : 'text-muted-foreground'}`}>
              {stat.sub}
            </div>
          </motion.div>
        ))}
