...
const RegulatoryView = () => {
  ...
  return (
    <div>
      <h2 className="font-display font-bold text-lg text-foreground mb-1">{t('regulatory.title')}</h2>
      <p className="font-body text-xs text-muted-foreground mb-5">{t('regulatory.subtitle')}</p>

      <div className="grid grid-cols-2 gap-3 mb-6">
        <motion.button initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card flex flex-col items-center justify-center py-5 hover:ring-1 hover:ring-accent/50">
          <Shield size={24} className="text-accent mb-2" />
          <span className="font-display text-sm font-bold text-foreground">TM30</span>
          <span className="font-display text-[10px] text-muted-foreground">{t('regulatory.auto_generate')}</span>
        </motion.button>
        <motion.button initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.05 }} className="glass-card flex flex-col items-center justify-center py-5 hover:ring-1 hover:ring-accent/50">
          <FileText size={24} className="text-primary mb-2" />
          <span className="font-display text-sm font-bold text-foreground">ภ.ง.ด.</span>
          <span className="font-display text-[10px] text-muted-foreground">{t('regulatory.export_tax')}</span>
        </motion.button>
      </div>
                ...
                <div key={item.id} className="glass-card p-0 overflow-hidden">
                  {img && (
                    <div className="relative h-24">
                      ...
                      <div className="absolute top-2 left-3"><span className="badge-destructive">{t('regulatory.urgent')}</span></div>
                    </div>
                  )}
                  <div className="p-4">
                    ...
                    <Button size="sm" className="mt-3 w-full bg-accent text-accent-foreground hover:bg-accent/90 font-display font-semibold text-xs">{t('regulatory.submit_now')}</Button>
                  </div>
                </div>
              );
            })}
