  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-1">
        <Users size={22} className="text-villa-ocean" />
        <h2 className="font-display font-bold text-xl text-foreground">{t('community.title')}</h2>
      </div>
      <p className="font-body text-sm text-muted-foreground mb-5">{t('community.subtitle')}</p>

      <div className="flex gap-2 mb-5 overflow-x-auto pb-1 scrollbar-hide">
        {regions.map((region, i) => (
          <button key={region.en} className={`font-display text-sm px-5 py-3 rounded-xl whitespace-nowrap transition-all font-semibold ${i === 0 ? 'bg-accent text-accent-foreground shadow-sm' : 'bg-card text-muted-foreground hover:bg-muted border border-border/50'}`}>
            {region[language]}
          </button>
        ))}
      </div>
          ...
          return (
            <motion.div key={post.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: idx * 0.06 }} className="glass-card p-4">
              <div className="flex items-start gap-3">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-accent to-villa-ocean text-primary-foreground flex items-center justify-center font-display text-sm font-bold flex-shrink-0 shadow-sm">{post.avatar}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="font-display text-base font-bold text-foreground">{post.author[language]}</span>
                    <span className="font-display text-xs text-muted-foreground">{post.time[language]}</span>
                  </div>
                  <div className="flex items-center gap-1.5 mb-2.5">
                    <span className="badge-muted flex items-center gap-1"><MapPin size={10} /> {post.region[language]}</span>
                    <span className="badge-muted">{catLabel}</span>
                  </div>
                  <p className="font-body text-base text-foreground leading-relaxed">{post.message[language]}</p>
                  <div className="flex items-center gap-5 mt-3 text-sm text-muted-foreground font-display">
                    <button className="flex items-center gap-1.5 hover:text-accent transition-colors"><ThumbsUp size={16} /> {post.likes}</button>
                    <button className="flex items-center gap-1.5 hover:text-villa-ocean transition-colors"><MessageCircle size={16} /> {post.replies}</button>
                  </div>
                </div>
              </div>
            </motion.div>
