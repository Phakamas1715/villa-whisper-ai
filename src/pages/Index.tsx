...
const Index = () => {
  ...
  const selectedItem: TimelineItem | null = mockTimeline.find((t) => t.id === selectedId) ?? null;
      ...
      <div className="md:ml-[68px] pb-20 md:pb-0">
        <div className="max-w-6xl mx-auto flex gap-5 p-4 lg:p-6 min-h-screen">
          ...
          {activeSection === 'timeline' && (
            <div className="hidden lg:block w-80 xl:w-[380px] flex-shrink-0 sticky top-6 self-start max-h-[calc(100vh-3rem)]">
      ...
      <AnimatePresence>
        {showDetailMobile && selectedItem && (
          <motion.div
            ...
            className="lg:hidden fixed inset-0 z-40 bg-card overflow-y-auto"
          >
            <DetailPane item={selectedItem} onClose={handleCloseDetail} />
          </motion.div>
        )}
